...
class UpdateProject extends Endpoint 
{
  ...
  if (!filter_has_var(INPUT_POST,'Title') || !filter_has_var(INPUT_POST,'Abstract') || !filter_has_var(INPUT_POST,'Main_Text') || !filter_has_var(INPUT_POST,'Partners') || !filter_has_var(INPUT_POST,'img') || !filter_has_var(INPUT_POST,'status')) {
    ...
    protected function initialiseSQL() {
        ...
        $sql = "UPDATE Projects SET Title = :title, Abstract = :abstract, Main_Text = :mainText, Partners = :partners, img = :img, status = :status WHERE id = :id";
        $this->setSQL($sql);
        $this->setSQLParams([
            'id' => $id,
            'title' => $title,
